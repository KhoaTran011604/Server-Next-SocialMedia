...
io.on("connection", (socket) => {
  ...
  socket.on("typing", ({ receiverId, senderName }) => {
    ...
    if (receiverSocketId) {
      io.to(receiverSocketId).emit("typing", { data: `Send form : ${senderName}` });
    }
  });

  // Khi user ngừng gõ
  socket.on("stop_typing", ({ receiverId, senderName }) => {
    const receiverSocketId = getReceiverSocketId(receiverId);
    if (receiverSocketId) {
      io.to(receiverSocketId).emit("stop_typing", { data: `Send form : ${senderName}` });
    }
  });
